...
class Connecting(Database):
    def __init__(self):
        super().__init__(password = "myr170500")

        # self.ip = ip
        self.dht = super().take_data("SELECT * FROM dht")
        self.suhu = self.dht[1]
        self.humid = self.dht[2]

class Tampilan(tk.Frame, Connecting):
    ...
    def build_label(self, text, row, column, columnspan, pady=0):
        tk.Label(self, text=text).grid(row=row, column=column, columnspan=columnspan, pady=pady)
    ...
    def build_widget(self):
        ...
        self.build_label(f"{self.suhu} C", 5, 0, 2, 15)
        self.build_label("Humidity", 4, 2, 2)
        self.build_label(f"{self.humid}%", 5, 2, 2, 15)
