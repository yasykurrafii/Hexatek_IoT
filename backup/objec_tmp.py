...
class ButtonToggle(tk.Tk, Database, Client):
    ...
    def __command_on(self):
        ...
        Client.connecting(self, self.ip)
        Client.send(self, message = f'on {self.gpio}')
        Client.close_conn(self)
        self.button.configure(bg = "green", command = self.__command_off)

    def __command_off(self):
        Client.connecting(self, self.ip)
        Client.send(self, message = f'off {self.gpio}')
        Client.close_conn(self)
...
class Tampilan(tk.Frame, Widget):
    ...
    def checking_connection(self):
        server = Server(host = self.ip, port = 9999)
        res = server.check_server()
        return res
